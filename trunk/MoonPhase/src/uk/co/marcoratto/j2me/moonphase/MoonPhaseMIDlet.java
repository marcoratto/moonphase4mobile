 ...
 *
 * This file is part of the project Moon Phases For Mobile.
 *
 * Moon Phases For Mobile is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 2 of the License, or
 * any later version.
 *
 * Moon Phases For Mobile is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Moon Phases For Mobile; if not, write to the Free Software
...
public class MoonPhaseMIDlet extends MIDlet implements CommandListener {
	...
	public void commandAction(Command c, Displayable s) {
		if (c == exitCommand) {
			...
			PictureForm.getInstance().show(this,
					images[(this.list.getSelectedIndex() % 4)],
					this.list.getString(this.list.getSelectedIndex()));
		}
	}
	...
	public void calculate(int year) {
		...
		// int offset1 = -((Calendar.getInstance().get(Calendar.ZONE_OFFSET) +
		// Calendar.getInstance().get(Calendar.DST_OFFSET)) / (60 * 1000)) / 60;

		int offset1 = -((TimeZone.getDefault().getOffset(1,
				theDate.get(Calendar.YEAR), theDate.get(Calendar.MONTH),
				theDate.get(Calendar.DAY_OF_MONTH),
				theDate.get(Calendar.DAY_OF_WEEK),
				theDate.get(Calendar.MILLISECOND)) + TimeZone.getDefault()
				.getRawOffset()) / (60 * 1000)) / 60;
		theDate.set(Calendar.DAY_OF_MONTH, 1);
		theDate.set(Calendar.MONTH, 7);
		// int offset2 = -theDate.getTimezoneOffset() / 60;
		int offset2 = -((TimeZone.getDefault().getOffset(1,
				theDate.get(Calendar.YEAR), theDate.get(Calendar.MONTH),
				theDate.get(Calendar.DAY_OF_MONTH),
				theDate.get(Calendar.DAY_OF_WEEK),
				theDate.get(Calendar.MILLISECOND)) + TimeZone.getDefault()
		...
		currentTime = (double) dat.get(Calendar.HOUR_OF_DAY)
				+ (double) dat.get(Calendar.MINUTE) / 60D
				+ (double) dat.get(Calendar.SECOND) / 3600D;
		currentJD = JD(dat.get(Calendar.DAY_OF_MONTH),
				dat.get(Calendar.MONTH) + 1, year, currentTime);
		...
		for (int i = 0; i < 15; i++) {
			 ...
			 * 
			 * if(T[i] < currentJD) { phase = currentJD -
			 * myMoonPhase.newMoonJD(); }
			 */
	...
	public double JD(int date, int month, int year, double STD) {
		...
		double MJD = A + B + (double) (int) (30.6D * (double) (month + 1))
				+ (double) date + STD / 24D;
		return MJD + 2400000D;
	}
}
